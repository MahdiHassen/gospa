...
//
// Wide read: the entry SRAM is split into STAGE1_BATCH banks, entry g -> bank
// g % STAGE1_BATCH at local address g / STAGE1_BATCH. The scanner streams the
// channel's non-zeros as one contiguous run [0, total) and reads one aligned
// batch [b*W, b*W+W) per cycle -- lane i comes from bank i at local address b,
// so all banks share the read address b. The last batch is masked by
// out_lane_valid when total is not a multiple of W.
//
...
//
// row_ptr lives in flops (small, read combinationally). out_x is the row,
// out_y the column. The external fill interface is unchanged (entries written
// at sequential global addresses; the bank split is internal).
// =============================================================================
...
module act_sram_scanner #(
    ...
    localparam int BANK_DEPTH = (N_NZ_MAX + W - 1) / W,
    localparam int BANK_AW   = (BANK_DEPTH   < 2) ? 1 : $clog2(BANK_DEPTH)
)(
    input  wire  logic                    clk,
    input  wire  logic                    rst_n,

    // -- Fill: activation SRAM (one CSR entry per write; sequential addr) -----
    ...
    logic [BANK_AW-1:0] bank_rd_addr;                   // shared read addr = batch idx
    logic               bank_rd_en;
    logic [ENT_W-1:0]   bank_rdata [0:W-1];

    // Fill address split: entry g -> bank (g % W), local (g / W).
    logic [$clog2(W+1)-1:0] fill_bank;
    ...
    // -------------------------------------------------------------------------
    // Entry SRAM, split into W banks. Bank b holds global entries g with
    // g % W == b, at local address g / W. All banks read the same local
    // address (= batch index) so one aligned batch is W parallel words.
    // -------------------------------------------------------------------------
    assign fill_bank  = fill_entry_addr % ($clog2(W+1))'(W);
    assign fill_local = BANK_AW'(fill_entry_addr / ENT_AW'(W));

    genvar b;
    /* verilator lint_off PINCONNECTEMPTY */
    generate
        for (b = 0; b < W; b++) begin : g_bank
            sram #(
                .DATA_WIDTH    (ENT_W),
                .ADDR_WIDTH    (BANK_AW),
                .USE_DUAL_PORT (1'b1),
                ...
                .clk         (clk),
                .rst_n       (rst_n),
                .a_en        (fill_entry_we && (fill_bank == ($clog2(W+1))'(b))),
                .a_we        (fill_entry_we && (fill_bank == ($clog2(W+1))'(b))),
                .a_addr      (fill_local),
    ...
    // Bank read address: issue batch 0 in S_ISSUE, pre-issue batch b+1 on accept.
    always_comb begin
        bank_rd_en   = 1'b0;
        bank_rd_addr = BANK_AW'(cur_b);
        unique case (state)
            S_ISSUE: begin
                bank_rd_en   = 1'b1;
                bank_rd_addr = BANK_AW'(cur_b);
    ...
    always_comb begin
        for (int i = 0; i < W; i++) begin
            ...
            out_value[i]      = bank_rdata[i][ENT_W-1 -: DATA_W];
            out_y[i]          = bank_rdata[i][IDX_W-1:0];
