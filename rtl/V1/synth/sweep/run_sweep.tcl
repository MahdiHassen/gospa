# =============================================================================
# run_sweep.tcl -- synthesize ONE V1 gospa config and append its utilisation
# GoSPA Project -- Team 19, ECE 720 (Spring 2026)
#
# V1 counterpart of rtl/V2/synth/sweep/run_sweep.tcl. Invoked once per config
# by run_sweep.sh:
#   vivado -mode batch -notrace -source run_sweep.tcl -tclargs <N_PE> <N_MULTS> <FIFO_D> [RUN_IMPL]
#
# NOTE vs the V2 sweep: V1 has a SINGLE FIFO_D parameter covering both FIFO-A
# and FIFO-B (V2 splits them into FIFO_D / FIFOB_D), so only FIFO_D is passed
# here. The swept axis is still "FIFO depth" in both versions.
#
# RUN_IMPL (optional, default 0): 0 = synth only (fast; utilisation is accurate
# post-synthesis), 1 = full place & route. Appends one row to sweep_results.csv.
# H=8, F=3, S=1 fixed (small, KV260-fitting) -- identical to the V2 sweep.
# =============================================================================
...
# ---- synthesize the config (V1: one FIFO_D for both FIFOs) -----------------
synth_design -top gospa -part $PART -mode out_of_context -flatten_hierarchy rebuilt \
    ...
    -generic N_PE=$NPE -generic N_MULTS=$NMULTS -generic FIFO_D=$FIFOD
