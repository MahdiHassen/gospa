#!/usr/bin/env bash
# =============================================================================
# run_sweep.sh -- drive the gospa resource sweep across N_PE / N_MULTS / FIFO_D
# GoSPA Project -- Team 19, ECE 720 (Spring 2026)
#
# Runs one Vivado synth per config (each independent), appending a row to
# sweep_results.csv. Then run:  python3 plot_sweep.py
#
#   ./run_sweep.sh            # synth-only (fast, ~1-2 min/config)
#   RUN_IMPL=1 ./run_sweep.sh # full place & route (matches RESULTS.md, slower)
# =============================================================================
set -e
cd "$(dirname "$0")"
...
for cfg in "${CONFIGS[@]}"; do
  ...
  echo "=================================================="
  echo "  synth: N_PE=$npe  N_MULTS=$nmults  FIFO_D=$fifod  (RUN_IMPL=$RUN_IMPL)"
  echo "=================================================="
